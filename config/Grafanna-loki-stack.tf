...
provider "kubernetes" {
  ...
  host  = data.digitalocean_kubernetes_cluster.kubernetes-api-cluster[0].endpoint

  token = data.digitalocean_kubernetes_cluster.kubernetes-api-cluster[0].kube_config[0].token

  cluster_ca_certificate = base64decode(

    data.digitalocean_kubernetes_cluster.kubernetes-api-cluster[0].kube_config[0].cluster_ca_certificate
